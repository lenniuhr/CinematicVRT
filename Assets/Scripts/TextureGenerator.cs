...
public class TextureGenerator : MonoBehaviour
{
    ...
    private void ReadWithFODICOM()
    {
        //DicomFile dicomFile = DicomFile.Open("C:/Users/lenna/Desktop/DicomConverter/input/2FA0BFF0.dicom");
        DicomFile dicomFile = DicomFile.Open("C:/Users/lenna/Desktop/DicomConverter/input/2FA0BFF0");
        //DicomFile dicomFile = DicomFile.Open("C:/Users/lenna/Desktop/DicomConverter/input/0002.DCM");

        DicomPixelData pixelData = DicomPixelData.Create(dicomFile.Dataset);

        Debug.Log("Original size: (" + pixelData.Width + ", " + pixelData.Height + ")");
        Debug.Log("Bit depth: " + pixelData.BitDepth);
        Debug.Log("Photometric Interpretation: " + pixelData.PhotometricInterpretation);
        Debug.Log("Color Space: " + pixelData.PhotometricInterpretation.ColorSpace);


        byte[] originalRawBytes = pixelData.GetFrame(0).Data;
        Debug.Log("Orignal size: " + originalRawBytes.Length + " bytes");

        IPixelData iPixelData = PixelDataFactory.Create(pixelData, 0);

        //double min = iPixelData.GetMinMax().Minimum;
        //double max = iPixelData.GetMinMax().Maximum;
        //Debug.Log("DICOM Range: [" + min + ", " + max + "]");

        Debug.Log("Size: (" + iPixelData.Width + ", " + iPixelData.Height + ")");

        Debug.Log("PixelData type: " + iPixelData.GetType());

        if (iPixelData is GrayscalePixelDataS16)
        {
            GrayscalePixelDataS16 grayscalePixelData = (GrayscalePixelDataS16)iPixelData;
            Debug.Log("Size: " + grayscalePixelData.Data.Length);

            grayscalePixelData.GetPixel(grayscalePixelData.Data.Length - 1, 0);
        }
        if (iPixelData is GrayscalePixelDataU8)
        {
            GrayscalePixelDataU8 grayscalePixelData = (GrayscalePixelDataU8)iPixelData;
            Debug.Log("Size: " + grayscalePixelData.Data.Length);

            grayscalePixelData.GetPixel(grayscalePixelData.Data.Length - 1000, 1);
        }
        ...
        for (int i = 0; i < iPixelData.Width; i++)
        {
            for (int j = 0; j < iPixelData.Height; j++)
            {
                //Console.WriteLine("{0}", Convert.ToSingle(iPixelData.GetPixel(i, j)));
            }
        }
    }

    private void ReadWithSharp()
    {
        byte[] pngBytes = File.ReadAllBytes("C:/Users/lenna/Desktop/DicomConverter/array");

        Debug.Log(pngBytes.Length);


        int size = Mathf.RoundToInt(Mathf.Sqrt(pngBytes.Length / 4));
        Debug.Log(size);
